...
import { generateState } from './app/login/generateState';
...
//   if (!refreshToken) return redirectToKeycloakAuth(request);

//   const decoded = decodeToken(refreshToken.value);
//   if (!decoded || decoded.exp < Date.now() / 1000)
//     return redirectToKeycloakAuth(request);
...
export function redirectToKeycloakAuth(request: NextRequest, origin: string) {
  ...
  // return NextResponse.redirect(new URL(`${authUrl}?${params.toString()}`));
...
// https://md-auth.ru/realms/medalist-realm/protocol/openid-connect/auth?response_type=code&state=EkyMhkroiYC4UEbSGJyIUNpyqS2R5a&client_id=medalist-client&scope=openid&code_challenge=bLWxC5eAUeZEX-vqk3JRHMSjSpMYIRwDJV5ja-uv7VY&code_challenge_method=S256&redirect_uri=http://localhost:3000/user/create
