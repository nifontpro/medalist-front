...
/**
 * !!!!!!!!!!!!!!!!!!
 * В переменных окружения заменить:
 * API_SERVER_URL=https://nmedalist.ru:8765/client на
 * API_SERVER_URL=https://nmedalist.ru:8765
 * т.к. теперь обращение к нескольким мс
 * !!!!!!!!!!!!!!!!!
 *
 * в API user, dept нужно во всех путях добавить обращение через функции: userUrl, deptUrl по аналогии с award, там сделал.
 */
...
export const galleryApi = createApi({
    ...
    endpoints: (build) => ({
         ...
         *            "createdAt",
         */
        getItemsByFolder: build.query<GalleryItem[], { folderId: number, baseRequest: BaseRequest }>({
            query: (request) => {
                return {
                    method: 'POST',
